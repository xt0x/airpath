...
describe("flight track response fixtures", () => {
  it("captures a /track response with an ordered position list", () => {
    ...
    const positions = currentTrack?.trackResponse.body.positions;
    if (positions === undefined) {
      throw new Error("current track fixture is missing positions");
    }
    ...
    expect(positions.map((position) => position.timestamp)).toEqual([
